...
func InstallHooks(projectRoot string) error {
	...
	for name, content := range hooks {
		...
		if err == nil {
			// File exists — only overwrite if it contains our marker
			s := string(existing)
			if !strings.Contains(s, "contracts check") && !strings.Contains(s, "Generated by contracts") {
				fmt.Fprintf(os.Stderr, "warn: %s already exists with custom content, skipping\n", path)
				continue
			}
		}
