...
func RunCommand(c *types.CommandCheck, cwd string) Result {
	...
	if exitCode != expected {
		return Result{false, fmt.Sprintf("exit code %d (expected %d)", exitCode, expected)}
	}

	if c.OutputMatches != "" {
		re, err := regexp.Compile(c.OutputMatches)
		if err != nil {
			return Result{false, fmt.Sprintf("invalid output_matches regex: %v", err)}
		}
		stdout := strings.TrimSpace(string(out))
		if !re.MatchString(stdout) {
			return Result{false, fmt.Sprintf("output %q did not match /%s/", stdout, c.OutputMatches)}
		}
	}
