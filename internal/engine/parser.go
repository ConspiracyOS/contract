...
// rawContract mirrors the YAML structure before custom unmarshalling of Scope.
...
var validTriggers = map[Trigger]bool{
	TriggerCommit: true, TriggerPR: true,
	TriggerMerge: true, TriggerSchedule: true,
}
...
func ParseContract(data []byte) (*Contract, error) {
	...
	if !validTriggers[raw.Trigger] {
		return nil, fmt.Errorf("contract %s: invalid trigger %q (must be commit|pr|merge|schedule)", raw.ID, raw.Trigger)
	}
	if len(raw.Checks) == 0 {
